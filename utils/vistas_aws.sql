...
create view vista_medicamentos_recetados_mes_aws as
SELECT s."sede_id",
       s."nombre" AS sede,
       t.medicamento,
       t.total_recetas
...
create view vista_medicos_mas_consultas_semana_aws as
SELECT 
    semana,
    medico,
    sede_nombre,
...
--consulta 3
create view vista_tiempo_prom_cita_y_diagnostico_aws as
SELECT
  c.sede_id,
  s.nombre AS sede,
  CONCAT(
...
ORDER BY c.sede_id;
--consulta 4 
create view vista_auditorias_ultimos_10_accesos_aws as
SELECT
  a.fecha_evento,
  a.accion,
  a.ip_origen,
...
create view vista_departamentos_comparten_equipamiento_otra_sede_aws as
SELECT
  e."equipamiento_id",
  e."nombre" AS equipo,
  e."categoria",
...
create view vista_pacientes_atendidos_enfermedad_sede_aws as
SELECT
  c."sede_id",
  s."nombre" AS sede,
