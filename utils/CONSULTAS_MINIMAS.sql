...
-- #### CONSULTA 1 -> Listar los medicamentos más recetados por sede en el último mes.
...
-- consulta 1 vista azure
create or replace view vista_medicamentos_recetados_mes_azure AS select * from dblink('host=sistemadesalud.postgres.database.azure.com dbname=sistemadesalud port=5432 user=Administrador password=Root12345', 
'select * from vista_medicamentos_mas_recetados_mes')
AS tl (sede_id int, sede varchar(100), medicamento varchar(100), total_recetas bigint);
-- consulta 1 vista aws
create or replace view vista_medicamentos_recetados_mes_aws AS select * from dblink('host=sistemadesalud2.c5s22qk407zg.us-east-2.rds.amazonaws.com dbname=sistemadesalud2 port=5432 user=postgres password=postgres', 
	'select * from vista_medicamentos_mas_recetados_mes')
AS tl (sede_id int, sede varchar(100), medicamento varchar(100), total_recetas bigint);

--consulta 1 vista unificada

create or replace view vista_medicamentos_recetados_mes_global as
select sede_id,sede,medicamento,total_recetas from vista_medicamentos_recetados_mes_local 
union all 
select sede_id,sede,medicamento,total_recetas from vista_medicamentos_recetados_mes_azure 
union all
select sede_id,sede,medicamento,total_recetas from vista_medicamentos_recetados_mes_aws
order by total_recetas DESC

select * from vista_medicamentos_recetados_mes_global

-- ##### CONSULTA 2 -> Mostrar los médicos con mayor número de consultas atendidas por semana.

--consulta 2 medicos con mayor numero de consultas en la semana 
select * from vista_medicos_mas_consultas_semana_local
create view vista_medicos_mas_consultas_semana as
SELECT 
    semana,
    medico,
    sede_nombre,
...
select * from vista_medicos_mas_consultas_semana_azure
	...
	create or replace view vista_medicos_mas_consultas_semana_azure AS select * from dblink('host=sistemadesalud.postgres.database.azure.com dbname=sistemadesalud port=5432 user=Administrador password=Root12345', 
	'select * from vista_medicos_mas_consultas_semana')
	...
	create or replace view vista_medicos_mas_consultas_semana_aws AS select * from dblink('host=sistemadesalud2.c5s22qk407zg.us-east-2.rds.amazonaws.com dbname=sistemadesalud2 port=5432 user=postgres password=postgres', 
	'select * from vista_medicos_mas_consultas_semana_aws')
	AS tl (semana text, medico text,sede_nombre character varying (100), ciudad text,consultas_atendidas bigint, ranking bigint);
...
create or replace view vista_medicos_mas_consultas_semana_global as
select * from vista_medicos_mas_consultas_semana_local
union all 
select * from vista_medicos_mas_consultas_semana_azure 
union all
select * from vista_medicos_mas_consultas_semana_aws
order by semana desc, ciudad asc, ranking asc
LIMIT ALL;

-- ##### CONSULTA 3 -> Reportar el tiempo promedio entre la cita y el registro de diagnóstico.
create view vista_tiempo_prom_cita_y_diagnostico_pereira as
SELECT
  c.sede_id,
  s.nombre AS sede,
  CONCAT(
...
ORDER BY c.sede_id;
	...
	select * from vista_tiempo_prom_cita_y_diagnostico_cali;
	create view vista_tiempo_prom_cita_y_diagnostico_cali AS select * from dblink('host=sistemadesalud2.c5s22qk407zg.us-east-2.rds.amazonaws.com dbname=sistemadesalud2 port=5432 user=postgres password=postgres', 
...
--consulta 3 azure
	select * from vista_tiempo_prom_cita_y_diagnostico_bogota;
	create view vista_tiempo_prom_cita_y_diagnostico_bogota AS select * from dblink('host=sistemadesalud.postgres.database.azure.com dbname=sistemadesalud port=5432 user=Administrador password=Root12345', 
	...
	create or replace view vista_tiempo_prom_cita_y_diagnostico_global as
	select * from vista_tiempo_prom_cita_y_diagnostico_pereira
	union all 
	select * from vista_tiempo_prom_cita_y_diagnostico_cali
	union all
	select * from vista_tiempo_prom_cita_y_diagnostico_bogota
...
-- ####### CONSULTA 4 -> Generar un informe de auditoría con los últimos 10 accesos a la tabla Historias_Clinicas.
...
--consulta 4aws
	select * from vista_auditorias_ultimos_10_accesos_cali;
	create view vista_auditorias_ultimos_10_accesos_cali AS select * from dblink('host=sistemadesalud2.c5s22qk407zg.us-east-2.rds.amazonaws.com dbname=sistemadesalud2 port=5432 user=postgres password=postgres', 
	...
	--consulta 4 azure
	select * from vista_auditorias_ultimos_10_accesos_bogota;
	
	create view vista_auditorias_ultimos_10_accesos_bogota AS select * from dblink('host=sistemadesalud.postgres.database.azure.com dbname=sistemadesalud port=5432 user=Administrador password=Root12345', 
	...
	create view vista_auditorias_ultimos_10_accesos_global as
...
-- ####### CONSULTA 5 -> Consultar los departamentos que comparten equipamiento con otra sede.
...
-- ####### CONSULTA 6 -> Calcular el total de pacientes atendidos por enfermedad y por sede.
...
ORDER BY c."sede_id", pacientes_unicos DESC, h."diagnostico";
	...
	--consulta 5 azure
	select * from vista_pacientes_atendidos_enfermedad_sede_azure limit 6;
	...
	drop view vista_pacientes_atendidos_enfermedad_sede_aws_global
	select * from vista_pacientes_atendidos_enfermedad_sede_global
	create or replace view vista_pacientes_atendidos_enfermedad_sede_global as
